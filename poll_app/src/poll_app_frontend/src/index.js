...
//Note you will use "poll_app_backend" in this JavaScript code a few times to call the backend
...
document.addEventListener('DOMContentLoaded', async (e) => {
  ...
  const question = await poll_app_backend.getQuestion();
  ...
  const voteCounts = await poll_app_backend.getVotes();
...
//via "await poll_app_backend.vote(selectedOption)"
pollForm.addEventListener('submit', async (e) => {
  ...
  const updatedVoteCounts = await poll_app_backend.vote(checkedValue);
...
resetButton.addEventListener('click', async (e) => {
    ...
    await poll_app_backend.resetVotes();
    const voteCounts = await poll_app_backend.getVotes();
    updateLocalVoteCounts(voteCounts);
